...
namespace WARS.DataAccessLayer
{
    public class StatementRunDAL : IStatementRunDAL
    {
        ...
        public DataSet UpdatestmtRunData(Array stmtsToRun, Array stmtsToRerun, Array stmtsToArchive,
    Array stmtsToReprintDetail, Array stmtsToReprintSummary, string stmtEndPeriod, string loggedUser, out Int32 iErrorId)
        {
            ds = new DataSet();
            try
            {
                ...
                OracleParameter stmts_to_reprintDetail = new OracleParameter();
                OracleParameter stmts_to_reprintSummary = new OracleParameter();
                OracleParameter stmt_end_period = new OracleParameter();
                ...
                stmts_to_reprintDetail.OracleDbType = OracleDbType.Int32;
                stmts_to_reprintDetail.Direction = ParameterDirection.Input;
                stmts_to_reprintDetail.CollectionType = OracleCollectionType.PLSQLAssociativeArray;
                if (stmtsToReprintDetail.Length == 0)
                {
                    stmts_to_reprintDetail.Size = 1;
                    stmts_to_reprintDetail.Value = new OracleDecimal[1] { OracleDecimal.Null };
                }
                else
                {
                    stmts_to_reprintDetail.Size = stmtsToReprintDetail.Length;
                    stmts_to_reprintDetail.Value = stmtsToReprintDetail;
                }
                orlCmd.Parameters.Add(stmts_to_reprintDetail);

                stmts_to_reprintSummary.OracleDbType = OracleDbType.Int32;
                stmts_to_reprintSummary.Direction = ParameterDirection.Input;
                stmts_to_reprintSummary.CollectionType = OracleCollectionType.PLSQLAssociativeArray;
                if (stmtsToReprintSummary.Length == 0)
                {
                    stmts_to_reprintSummary.Size = 1;
                    stmts_to_reprintSummary.Value = new OracleDecimal[1] { OracleDecimal.Null };
                }
                else
                {
                    stmts_to_reprintSummary.Size = stmtsToReprintSummary.Length;
                    stmts_to_reprintSummary.Value = stmtsToReprintSummary;
                }
                orlCmd.Parameters.Add(stmts_to_reprintSummary);
