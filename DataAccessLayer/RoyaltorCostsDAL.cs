...
namespace WARS.DataAccessLayer
{
    public class RoyaltorCostsDAL : IRoyaltorCostsDAL
    {
        ...
        public DataSet SaveCost(string royaltor, string statementPeriod, string fromDate, string toDate,string userCode, Array costList, out Int32 iErrorId)
        {
            ds = new DataSet();
            try
            {
                ...
                OracleParameter pCostList = new OracleParameter();
                OracleParameter pCostData = new OracleParameter();
                orlCmd = new OracleCommand("pkg_royaltor_costs_screen.p_save_cost_data", orlConn);
                orlCmd.CommandType = System.Data.CommandType.StoredProcedure;
                ...
                pCostList.OracleDbType = OracleDbType.Varchar2;
                pCostList.Direction = ParameterDirection.Input;
                pCostList.CollectionType = OracleCollectionType.PLSQLAssociativeArray;
                if (costList.Length == 0)
                {
                    pCostList.Size = 1;
                    pCostList.Value = new OracleDecimal[1] { OracleDecimal.Null };
                }
                else
                {
                    pCostList.Size = costList.Length;
                    pCostList.Value = costList;
                }
                orlCmd.Parameters.Add(pCostList);
            ...
            string suppName, string projCode, string invNum, string isDeleted, out Int32 iErrorId)
        {
            ...
            try
            {
                ...
                OracleParameter pProjCode = new OracleParameter();
                OracleParameter pInvNum = new OracleParameter();
                OracleParameter pIsDeleted = new OracleParameter();
                OracleParameter pCostData = new OracleParameter();
                ...
                pIsDeleted.OracleDbType = OracleDbType.Varchar2;
                pIsDeleted.Direction = ParameterDirection.Input;
                pIsDeleted.Value = isDeleted;
                orlCmd.Parameters.Add(pIsDeleted);
